...
from app.models import ResultPayload
...
# Test the /ml/data/{task_id} endpoint by mocking its logic
@patch("requests.post")
def test_process_data(mocked_requests_post):
    task_id = 12345

    # Mock the response from the Node.js server
    mocked_requests_post.return_value.status_code = 200
    mocked_requests_post.return_value.json.return_value = {"status": "Success"}

    # Simulate sending the POST request to /ml/data/{task_id}
    response = client.post(f"/ml/data/{task_id}")
    ...
    expected_url = f"{settings.NODE_WEBHOOK_URL}/ml/result"

    # Assert that the request to the Node.js server was made to the expected URL
    mocked_requests_post.assert_called_with(expected_url, json=ANY)
